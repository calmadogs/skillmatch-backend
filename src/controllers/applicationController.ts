...
export const getAllApplications = async (req: Request, res: Response) => {
  ...
  // Conversão correta dos parâmetros
  const pid = projectId ? Number(projectId) : null;
  const fid = freelancerId ? Number(freelancerId) : null;

  try {
    // ADMIN pode listar tudo
    if (loggedUser.role === "ADMIN") {
      const filters: any = {};
      if (fid) filters.freelancerId = fid;
    ...
    // FREELANCER vê apenas suas próprias aplicações
    ...
    // CLIENTE vê aplicações dos projetos dele
    if (loggedUser.role === "CLIENT") {
      ...
      // Se enviou projectId, validar corretamente
      ...
      const apps = await prisma.application.findMany({
        where: {
          projectId: pid ? pid : { in: projectIds },
          ...(status ? { status: String(status) } : {}),
        },
        include: { freelancer: true, project: true },
      });
...
export const createApplication = async (req: Request, res: Response) => {
  const { freelancerId, projectId, status, message } = req.body;
  const loggedUser = (req as any).user;
